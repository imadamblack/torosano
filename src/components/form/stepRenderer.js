...
export default function StepRenderer({
  ...
  switch (step.type) {
    ...
    case 'opt-in':
      return (
        <>
          <div className="flex flex-col bg-neutral-800 p-12 rounded-3xl gap-4">
            <div>
              <p className="ft-4 text-white sans font-bold" dangerouslySetInnerHTML={{__html: step.title}}/>
              {step.description && (
                <p className="ft-2 text-white my-4" dangerouslySetInnerHTML={{__html: step.description}}/>
              )}
            </div>
          ...
          <div className="mt-4">
            <p className="-ft-3 text-center text-brand-1">Tus datos no serán compartidos.<br/>Al continuar aceptas
              nuestra&nbsp;
